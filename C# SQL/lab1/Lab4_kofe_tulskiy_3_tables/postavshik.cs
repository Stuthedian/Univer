...
namespace Lab4_kofe_tulskiy
{
	public partial class postavshik : Form
	{
		...
		private void Save_button_Click(object sender, EventArgs e)
        {
            ...
            command.CommandText = "kol_postavshik";
            ...
            {
                command.Parameters.Clear();
                command.CommandText = "vvod_postavshik";
                command.Parameters.Add("@idp", SqlDbType.Int);
                command.Parameters["@idp"].Direction = ParameterDirection.Output;
                command.Parameters.Add("@name", SqlDbType.VarChar);
                command.Parameters["@name"].Value = textBox1.Text.Trim();
                try
                { connection.Open(); }
                ...
                connection.Close();
                dataGridView1.Rows.Add(command.Parameters["@idp"].Value.ToString(), textBox1.Text);
                textBox1.Text = "";
            }
            else
            {
                command.Parameters.Clear();
                command.CommandText = "update_postavshik";
                command.Parameters.Add("@idp", SqlDbType.Int);
                command.Parameters.Add("@name", SqlDbType.VarChar);
                command.Parameters["@idp"].Value = dataGridView1.CurrentRow.Cells[0].Value.ToString();
		...
		private void Delete_button_Click(object sender, EventArgs e)
        {
            ...
            command.Parameters.Clear();
            command.CommandType = CommandType.StoredProcedure;
            command.CommandText = "kol1_postavshik";
            command.Parameters.Add("@idp", SqlDbType.Int);
            command.Parameters["@idp"].Value = dataGridView1.CurrentRow.Cells[0].Value.ToString();
            try
            { connection.Open(); }
            ...
            if (kol > 0)
            {
                MessageBox.Show("Удаление не возможно. Есть записи о поставках данного поставщика");
            ...
            {
                command.Parameters.Clear();
                command.CommandText = "del_postavshik";
                command.Parameters.Add("@idp", SqlDbType.Int);
