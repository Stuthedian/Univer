...
IpDelegate::IpDelegate(QObject *parent, bool valid):QItemDelegate(parent), validate(valid)
{
    uniqueData = true;  // по умолчанию
}

QWidget* IpDelegate::createEditor(QWidget *parent,const QStyleOptionViewItem &option, const QModelIndex &index) const {
 QLineEdit* e = new QLineEdit(parent);
 if (validate) {
  QString num = "([0-9]|[1-9][0-9]|1[0-9][0-9]|2[0-4][0-9]|25[0-5])";
  QRegExp rx(num+"\\."+num+"\\."+num+"\\."+num);//CHANGE
...
void IpDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option,
...
void IpDelegate::setEditorData(QWidget *editor, const QModelIndex &index) const {
 QLineEdit* e = qobject_cast<QLineEdit*>(editor);
 e->setText(index.model()->data(index).toString());
}

void IpDelegate::setModelData(QWidget *editor, QAbstractItemModel *model,
     const QModelIndex &index) const {
 ...
 if (!isUnique) {
     //qobject_cast<QPushButton*>(editor->layout()->itemAt(2)->widget())->clicked();
...
bool IpDelegate::editorEvent(QEvent *event, QAbstractItemModel *model,
...
void IpDelegate::updateEditorGeometry(QWidget *editor,
...
QSize IpDelegate::sizeHint(const QStyleOptionViewItem &option,
      const QModelIndex &index) const {
 return QSize(option.rect.width(), 20);
}

void IpDelegate::setUnique(bool isUnique) {
 uniqueData = isUnique;
}
