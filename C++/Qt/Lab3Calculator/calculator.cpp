...
Calculator::Calculator(QWidget *parent) : QWidget(parent)
{
    displaystring = new QLabel("");
     ...
     QGridLayout *myLayout = new QGridLayout;
      ...
      m_pmnu->addAction("Copy");
      m_pmnu->addAction("Paste");
      m_pmnu->addAction("Cut");
      connect(m_pmnu,
...
void Calculator::slotActivated(QAction* pAction)
{
    //QString strColor = pAction->text().remove("&");
    //displaystring->setText("");
    //setHtml(QString("<BODY BGCOLOR=%1></BODY>").arg(strColor));
    QClipboard* c = QApplication::clipboard();
    if (pAction->text() == "Copy")
    {
        c->setText(displaystring->text());
    }
    else if(pAction->text() == "Paste")
    {
        if(c->mimeData()->hasText())
        {
            bool ok;
            QString a = c->mimeData()->text();
            double b = a.toDouble(&ok);
            if(ok)
            {
                displaystring->setText(QString::number(b));
            }

        }
    }
    else if(pAction->text() == "Cut")
    {
